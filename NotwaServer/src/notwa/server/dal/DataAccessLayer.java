...
import notwa.server.sql.SimpleSqlFilter;
import notwa.server.sql.Sql;
import notwa.common.ParameterSet;
import notwa.server.sql.SqlBuilder;
import notwa.server.sql.SqlFilter;
...
public abstract class DataAccessLayer<TObject extends BusinessObject, TCollection extends BusinessObjectCollection<TObject>> {
    ...
    protected DatabaseConnectionInfo ci;
     ...
     */
    public DataAccessLayer(DatabaseConnectionInfo ci, Context context) {
    ...
    public void update(TCollection boc) {
        if (boc.getDataContextReference() == null && !boc.isUpdateRequired()) {
            return;
        }

        SmartResultSet srs = (SmartResultSet) boc.getDataContextReference();
    ...
    protected Integer getLastUniqeIdentifier(TObject bo) throws DalException {
        try {
            Integer potentialPrimaryKey = getConnection().executeScalar(getHighestUniqeIdentifierSql(bo));
            return (potentialPrimaryKey == null) ? 0 : potentialPrimaryKey;
        } catch (Exception ex) {
